    ...
    const insights = [
      {
        ...
        trend:
          pattern.totalTransactions > 500
            ? "high"
            : pattern.totalTransactions > 100
            ? "medium"
            : "low",
      },
      {
        ...
        trend:
          pattern.gasEfficiency > 0.8
            ? "excellent"
            : pattern.gasEfficiency > 0.6
            ? "good"
            : "needs_improvement",
      },
      {
      ...
      },
      {
        ...
        trend:
          pattern.protocolUsage.size > 5
            ? "diversified"
            : pattern.protocolUsage.size > 2
            ? "moderate"
            : "limited",
      },
    ];
    ...
    const insights = [
      {
        ...
        trend:
          staking.stakingScore > 0.7
            ? "strong"
            : staking.stakingScore > 0.4
            ? "moderate"
            : "weak",
      },
      {
        ...
        trend:
          parseFloat(staking.totalStaked) > 1e19
            ? "large"
            : parseFloat(staking.totalStaked) > 1e18
            ? "medium"
            : "small",
      },
      {
        ...
        value: `${(parseFloat(staking.totalRewardsClaimed) / 1e18).toFixed(
          4
        )} ETH`,
        description: "Total rewards harvested",
        score: Math.min(parseFloat(staking.totalRewardsClaimed) / 1e19, 1),
        trend:
          parseFloat(staking.totalRewardsClaimed) > 1e18 ? "active" : "passive",
      },
      {
        type: "duration",
        title: "Avg Duration",
        value: `${(staking.avgStakingDuration / (24 * 60 * 60 * 1000)).toFixed(
          0
        )} days`,
        description: "Average staking period",
        score: Math.min(
          staking.avgStakingDuration / (90 * 24 * 60 * 60 * 1000),
          1
        ),
        trend:
          staking.avgStakingDuration > 30 * 24 * 60 * 60 * 1000
            ? "long_term"
            : "short_term",
      },
    ];
    ...
    const factors = [
      {
        ...
        severity:
          profile.liquidationBehavior.totalLiquidations > 3
            ? "high"
            : profile.liquidationBehavior.totalLiquidations > 0
            ? "medium"
            : "low",
      ...
      },
      {
        ...
        severity:
          profile.overallRiskScore > 0.7
            ? "high"
            : profile.overallRiskScore > 0.4
            ? "medium"
            : "low",
      ...
      },
      {
        ...
        severity:
          profile.dataCompleteness < 0.5
            ? "high"
            : profile.dataCompleteness < 0.8
            ? "medium"
            : "low",
        value: profile.dataCompleteness,
        description: `${(profile.dataCompleteness * 100).toFixed(
          1
        )}% data coverage`,
    ...
    const trends = [
      {
        ...
        trend:
          profile.transactionPattern.frequency > 50 ? "increasing" : "stable",
        change: "+12%",
        description:
          "Transaction frequency has increased over the past 30 days",
      },
      {
        category: "Staking Behavior",
        trend:
          profile.stakingBehavior.stakingScore > 0.6 ? "improving" : "stable",
        change: "+8%",
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">
          Enter an address above to analyze behavior patterns
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from(
            behaviorProfile.transactionPattern.protocolUsage.entries()
          ).map(([protocol, usage], index) => {
            const maxUsage = Math.max(
              ...Array.from(
                behaviorProfile.transactionPattern.protocolUsage.values()
              )
            );
